...
const productController = {
  ...
  async createProduct(req, res, next) {
    try {
      ...
      if (!productData.category || productData.category === '' || productData.category === 'undefined') {
        const defaultCategory = await Category.findOne();
        if (defaultCategory) {
          productData.category = defaultCategory._id;
        } else {
          return res.status(400).json({ success: false, message: 'Category is required. Please create a category first.' });
        }
      }
      
      // ✅ Auto-fill description
      if (!productData.description || productData.description.trim() === '') {
        productData.description = productData.name || 'No description';
      }
      ...
      if (req.files && req.files.length > 0) {
        ...
        productData.images = [{ public_id: 'placeholder', url: 'https://via.placeholder.com/500x500/4F46E5/FFFFFF?text=No+Image', isMain: true }];
      }
      ...
      if (productData.comparePrice) productData.comparePrice = parseFloat(productData.comparePrice) || undefined;
      if (productData.stock) productData.stock = parseInt(productData.stock) || 0;
      if (productData.isActive === 'true' || productData.isActive === true) productData.isActive = true;
      else productData.isActive = false;
      if (productData.isFeatured === 'true' || productData.isFeatured === true) productData.isFeatured = true;
      ...
      await auditService.log({
        admin: req.admin._id, action: 'create', resource: 'product', resourceId: product._id,
        details: { name: product.name, sku: product.sku }, ipAddress: req.ip, userAgent: req.get('user-agent'),
      }).catch(err => logger.error('Audit log failed:', err));
  ...
  async updateProduct(req, res, next) {
    try {
      ...
      if (req.files && req.files.length > 0) {
        updateData.images = req.files.map((file, index) => ({
          public_id: file.filename, url: `/uploads/products/${file.filename}`,
          isMain: req.body.mainImageIndex === index.toString(),
        }));
      }
      if (typeof updateData.tags === 'string') updateData.tags = updateData.tags.split(',').map(t => t.trim()).filter(t => t);
      if (updateData.price) updateData.price = parseFloat(updateData.price);
      if (updateData.comparePrice) updateData.comparePrice = parseFloat(updateData.comparePrice);
      if (updateData.stock) updateData.stock = parseInt(updateData.stock);
      if (typeof updateData.isActive === 'string') updateData.isActive = updateData.isActive === 'true';
      ...
      await auditService.log({
        admin: req.admin._id, action: 'update', resource: 'product', resourceId: req.params.id,
        details: { updatedFields: Object.keys(req.body) }, ipAddress: req.ip, userAgent: req.get('user-agent'),
      }).catch(err => logger.error('Audit log failed:', err));
      
      res.status(200).json({ success: true, data: product, message: 'Product updated successfully' });
    } catch (error) {
      if (error.code === 11000) return next(new AppError('SKU already exists', 400));
      next(error);
    }
  },
  ...
  async updateStock(req, res, next) {
    try {
      const product = await Product.findByIdAndUpdate(req.params.id, { stock: parseInt(req.body.stock) }, { new: true });
      if (!product) throw new AppError('Product not found', 404);
      res.status(200).json({ success: true, data: product, message: 'Stock updated' });
    } catch (error) { next(error); }
  },

  async getLowStockProducts(req, res, next) {
    try {
      const products = await Product.find({ stock: { $lte: parseInt(req.query.threshold || 10) }, isActive: true });
