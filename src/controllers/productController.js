...
const productController = {
  ...
  async createProduct(req, res, next) {
    try {
      ...
      // ✅ Handle category - if not provided, use first available category
      if (!productData.category || productData.category === '' || productData.category === 'undefined') {
        ...
        if (defaultCategory) {
          ...
          return res.status(400).json({
            success: false,
            message: 'Category is required. Please create a category first.',
          });
        }
      }
      
      // ✅ Handle description - if empty, use product name as fallback
